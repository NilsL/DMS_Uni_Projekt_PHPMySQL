<?php if (!defined('BASEPATH'))
    ...
    class Dokument extends MY_Controller {
        ...
        public function open($dok_id) {
            ...
            $query = $this->keywordmodel->getKeywords($dok_id);
            if ($query) {
                $data['keywords'] = implode(", ", $query);
            }

            $this->load->view('modal.php', $data);
        }

        public function edit($dok_id) {
            $query = $this->projektmodel->getProjekte();
            if ($query) {
                $data['projekte'] = $query;
            }
            ...
            $query = $this->dokumentmodel->getDokumente($dok_id);
            if ($query) {
                $data['dokument'] = $query;
            } else {
                $data['message'] = "Document not found in database. Please contact admin!";
            }

            $this->load->view('editDokument.php', $data);
        }
        ...
        public function update() {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $dok_id = $this->dokumentmodel->updateDokument($dok_id, $dokument);
                $this->keywordmodel->removeBinding($dok_id);
